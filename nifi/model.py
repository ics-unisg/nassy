...
with torch.no_grad():
    ...
    model2 = NN(17,2)
    model2.load_state_dict(torch.load('/code/nifi/model/torch', map_location=device))
    model2.eval()
    data_to_predict = df.drop(['subject', 'task', 'b_start', 'b_duration', 'e_start', 'e_duration', 'e_distance'], axis=1).to_numpy()[0]
    ...
    print(f'subject,task,prediction_class,b_start,e_start')
    print(f'{df.iloc[0]["subject"]},{df.iloc[0]["task"]},{predicted_class},{b_start},{e_start}')